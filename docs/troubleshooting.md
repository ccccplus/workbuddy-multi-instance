...
> 📌 **2026-09-20 补记**：5.5.6 实测两侧均**不再生成** `BundleMigration` 目录（官方疑似改用别的更新缓存机制，也可能被系统清理过）。
> 但这**不代表这一步能省** —— `darwinBundleIdentifier` 还决定其它路径（如偏好设置 plist 文件名），照改不误。
> 自检脚本已同步调整：主实例也无此目录时判"该版本不适用"，不再误报。
...
| 插件凭据隔离（补丁后） | ✅ 实测 |
| 跨版本升级后登录态保留 | ✅ 实测（5.5.2 → 5.5.6，副本启动后自动刷新会话文件，无需重新登录） |
| 服务端风控 | ⚠️ 无法从客户端代码判断，实测未触发，风险自担 |

方案在 **5.5.2 / 5.5.3 / 5.5.6** 三个版本上实测通过；5.5.6 的三处补丁点（`productName` / `darwinBundleIdentifier` / `authentication.id`）**依然在原位**，升级脚本无需改写。
